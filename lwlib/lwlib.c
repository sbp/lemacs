...
copy_scrollbar_values (widget_value *val, widget_value *copy)
{
  ...
  if (val->scrollbar_data)
    {
      scrollbar_values *val_sb = val->scrollbar_data;
      scrollbar_values *copy_sb = copy->scrollbar_data;

      copy_sb->line_increment = val_sb->line_increment;
      copy_sb->page_increment = val_sb->page_increment;
      copy_sb->minimum = val_sb->minimum;
      copy_sb->maximum = val_sb->maximum;
      copy_sb->slider_size = val_sb->slider_size;
      copy_sb->slider_position = val_sb->slider_position;
      copy_sb->scrollbar_height = val_sb->scrollbar_height;
      copy_sb->scrollbar_pos = val_sb->scrollbar_pos;
    }
  else
...
merge_scrollbar_values (widget_value *old, widget_value *new)
{
  ...
  if (new->scrollbar_data && !old->scrollbar_data)
	...
	{
	  changed = True;
	  old_sb->line_increment = new_sb->line_increment;
	}
      if (old_sb->page_increment != new_sb->page_increment)
	{
	  changed = True;
	  old_sb->page_increment = new_sb->page_increment;
	}
      if (old_sb->minimum != new_sb->minimum)
	{
	  changed = True;
	  old_sb->minimum = new_sb->minimum;
	}
      if (old_sb->maximum != new_sb->maximum)
	{
	  changed = True;
	  old_sb->maximum = new_sb->maximum;
	}
      if (old_sb->slider_size != new_sb->slider_size)
	{
	  changed = True;
	  old_sb->slider_size = new_sb->slider_size;
	}
      if (old_sb->slider_position != new_sb->slider_position)
	{
	  changed = True;
	  old_sb->slider_position = new_sb->slider_position;
	}
      if (old_sb->scrollbar_height != new_sb->scrollbar_height)
	{
	  changed = True;
	  old_sb->scrollbar_height = new_sb->scrollbar_height;
	}
      if (old_sb->scrollbar_pos != new_sb->scrollbar_pos)
	{
	  changed = True;
	  old_sb->scrollbar_pos = new_sb->scrollbar_pos;
	}
    }
