...
static Widget
make_scrollbar (widget_instance *instance)
{
  Arg al[20];
  int ac = 0;
  Dimension width;
  ...
  XtVaSetValues (scrollbar, XmNborderWidth, 0, 0);
  ...
  if (0
  ...
  else
    {
	 ...
	 popdown instead of destroy, but it can't hurt. */
      ...
      /*
       * Why the special case for the scrollbars?  The Pane widget which
       * is handling these doesn't react well to losing all of its
       * children while it is managed.  However, unmanaging while there
       * is more than 1 child remaining sends it into an infinite loop.
       * Unmanaging it while there is 1 child left causes a seg fault
       * 75% of the time.  So we solve the problem by simply not
       * destroying the last child.  It will get destroyed by its parent.
       */
      if (XtClass (widget) == xmScrollBarWidgetClass)
	{
	  Cardinal num_children;

	  XtVaGetValues (XtParent (instance->widget),
			 XmNnumChildren, &num_children, 0);

	  /* Apparently the scrollbar is made up of 3 different widgets */
	  if (num_children != 3)
