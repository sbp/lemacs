...
(defvar shell-pathname-pattern "[ \t\n()<>&|;]"
  "*Regexp used by shell name completion to mark path name boundaries.")

(defvar shell-filename-pattern "[~/]"
  "*Regexp used by shell name completion to mark file name boundaries.
Used in conjunction with shell-pathname-pattern.")
...
editing control characters, provide file-name completion, and
allow you to move a command at a time.
\\{shell-mode-map}
...
(if shell-mode-map
  ...
  (define-key shell-mode-map "\C-i" 'shell-complete-file-name)
  (define-key shell-mode-map "\C-m" 'shell-send-input)
  (define-key shell-mode-map "\en" 'shell-next-command)
  (define-key shell-mode-map "\ep" 'shell-prev-command)
...
(defun make-shell (name program &optional startfile &rest switches)
  (let ((buffer (get-buffer-create (concat "*" name "*")))
    ...
    buffer))

(defvar shell-set-directory-error-hook nil
  "List of functions called with no arguments when shell-send-input
...
This regexp should start with \"^\"."
  ...
  (condition-case ()
	...
	(shell-set-directory))
    (error (run-hooks 'shell-set-directory-error-hook)))
...
(defun shell-unduplicate-slashes (string)
...
Sends the current line as input first."
  (interactive)
  (shell-send-input)
  (process-send-eof))

(defun shell-next-command ()
  "Move to beginning of next shell command."
  (interactive)
  (re-search-forward shell-prompt-pattern nil t))

(defun shell-prev-command ()
  "Move to beginning of previous shell command."
  (interactive)
  (beginning-of-line)
...
(defun kill-output-from-shell ()
  "Kill output from shell since last input."
  (interactive)
  (goto-char (point-max))
  (re-search-backward shell-prompt-pattern nil t)
...
(defun kill-shell-input ()
  ...
  (kill-region (process-mark (get-buffer-process (current-buffer)))
	       (point)))

(defun shell-complete-file-name ()
  "Complete the filename ending at point."
  (interactive)
  (let ((shell-expand-name nil)
	...
	(if (equal shell-expand-try shell-expand-file)
	  ...
	  (progn
	    ...
	    (re-search-backward
	     (if (string-match "~" shell-expand-file)
		 (concat "/\\|"
			 shell-pathname-pattern)
...
(if inferior-lisp-mode-map
  ...
  (set-keymap-parent inferior-lisp-mode-map shell-mode-map)
