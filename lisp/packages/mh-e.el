...
(defun mh-refile-a-msg (msg destination)
  ...
  (save-excursion
    ...
    (cond ((looking-at mh-deleted-msg-regexp)
	  ...
	  ((looking-at mh-refiled-msg-regexp)
	   (message "Message %d is already refiled.  Copying..." msg)
	   (mh-exec-cmd "refile"
			(mh-get-msg-num t) "-link"
			"-src" mh-current-folder (symbol-name destination))))
    (mh-set-folder-modified-p t)
...
(defun mh-scan-folder (folder range)
  ...
  (mh-when (zerop (count-lines (point-min) (point-max)))
    (if (equal range "all")
...
(defun mh-regenerate-headers (range)
  ...
  (let ((folder mh-current-folder))
    ...
    (with-mh-folder-updating (nil)
      ...
      (mh-make-folder-mode-line (if (equal range "all") nil "select")))
...
(defun mh-make-folder-mode-line (&optional annotation)
  ...
  (save-excursion
    ...
    (goto-char (point-max))
    (let ((lines-checked 0))
      (while (and (< (setq lines-checked (1+ lines-checked)) 5)
		  (not (setq mh-last-msg-num (mh-get-msg-num nil))))
	(forward-line -1)))
