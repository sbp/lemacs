...
(put 'file-part-error 'error-conditions '(error file-part-error))
(put 'file-part-error 'error-message "File part error")
...
BUFFER defaults to the current buffer if not specified."
  ...
  (save-excursion
    ...
    (and (or no-ask
	     ...
	     (y-or-n-p (gettext
			"Buffer has modified file parts; kill anyway? ")))
...
(defun file-part-check-attached (x)
  (cond ((null x) nil)
	((extent-property x 'detached)
	 ...
	 (message (gettext "File part has become detached."))
	 nil)
	(t)))
    ...
    (and (file-part-check-attached x)
	 (let ((b (extent-start-position x))
	   ...
	   (save-excursion
	     ...
	     (set-buffer buf)
	     (set-buffer-modified-p nil)
	     (message (format (gettext "Wrote file part %s on %s")
			      (buffer-name buf)
...
BUFFER defaults to the current buffer if not specified."
  ...
  (let ((alist file-part-extent-alist)
	...
	(and (buffer-modified-p buf)
	     (or arg
		 (y-or-n-p (format (gettext "Save file part %s on %s? ")
    ...
    (and (file-part-check-attached x)
	 (let ((master (extent-buffer x)))
	   (and
		...
		(yes-or-no-p
		 (format
		  (gettext "Revert file part from master buffer %s? ")
		  (buffer-name master))))
	    (progn
	      (erase-buffer)
