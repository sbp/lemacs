...
;;;; Written by Eric Eide, last modified on December 29, 1993.
...
;;;;   However, I have received reports that scroll-window-in-place is often
;;;;   confused by VM 5's message scrolling commands.  I do not yet use VM 5
;;;;   myself and my attempts to remotely diagnose the problems have failed.
;;;;   When I upgrade I'll fix the bugs, unless somebody else beats me to it.
;;;;
...
;;;;   is a moot problem, as I am unaware of any process filters that invoke
;;;;   scrolling commands.
...
point could otherwise move the full number of lines."
  (let* (;; Make sure that the user doesn't quit in the middle and leave us
    ...
    (setq scroll-previous-window window
	  ;; scroll-boundary-error-command nil not necessary.
...
running sequence of scrolling commands will probably be broken anyway."
  (if (if (eq last-command t)
	...
	;; Otherwise, last-command must belong to some group of "in place"
	;; scrolling commands.
