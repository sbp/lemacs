...
(defconst dired-extra-version (substring "$Revision: 1.182 $" 11 -2)
  "$Id: dired-x.el,v 1.182 1992/02/14 15:59:09 sk RelBeta $")
...
;;    |$Date: 92/03/01 14:27:40 $|$Revision: 1.182 $|
...
(provide 'dired-extra)			; note that you'll need to
...
See variable `texinfo-unclean-extensions' and `texinfo-unclean-extensions'."
  (interactive)
...
Defaults to the elements of `completion-ignored-extensions' and
`texinfo-unclean-extensions'.")
...
"
     ...
     (let ((regexp "^\\(  \\)?\\([^ \n\r]*\\)\\(:\\)[\n\r]")
       ...
       (cond ((looking-at regexp)
		...
		(if (looking-at "^  wildcard ")
		    (expand-file-name (buffer-substring (match-end 0)
							(progn (end-of-line)
							       (point)))
				      dir)
  ...
  ;; mimic `dired-noselect':
  (setq dirname (expand-file-name (directory-file-name dirname)))
  (if (file-directory-p dirname)
      (setq dirname (file-name-as-directory dirname)))
  ;; mimic `dired-internal-noselect':
...
to put saved dired buffers automatically into virtual dired mode."
  (call-interactively 'dired-virtual))
...
Like function `comint::background' but in the current Tree Dired directory."
  (interactive "s%% ")
  (comint::background (concat "cd " (default-directory) "; " cmd))
  ;; This somehow changes the Dired buffer's default-directory!
;  (let ((default-directory (default-directory)))
;    (comint::background cmd))
)
