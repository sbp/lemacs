...
(if (null c-mode-map)
    (progn
      ...
      (set-keymap-name c-mode-map 'c-mode-map)
...
;;; statements.
(defvar c-switch-label-regexp
  (purecopy "case[ \t'/(]\\|default\\(\\S_\\|'\\)"))
...
preserving the comment indentation or line-starting decorations."
  ...
  (let* ((first-line (save-excursion
                       ...
                       (and (looking-at comment-start-skip)
                            (point))))
         (comment-start-place first-line))
    (if (or first-line
	 ...
	 (contain-stack
	  (list (if endpos
			...
			(condition-case nil
			  ...
			  (error
;;;>>> Commented-out in lemacs, present in FSFmacs
;                          (setq opoint funbeg)
                           (point)))))
		  (point))))
	 (case-fold-search nil)
...
;(defun indent-c-find-real-comment (state)
;  (let ((win nil))
;    (while (and (not win)
;		(re-search-forward comment-start-skip
