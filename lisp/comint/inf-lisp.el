...
(cond ((not inferior-lisp-mode-map)
       ...
       ;;>>> ARRGG!  Need multiple parents
       ;(set-keymap-parent inferior-lisp-mode-map shared-lisp-mode-map)
    ;; This used to be done by calling `lisp-mode-commands' in lisp-mode.el
    (define-key inferior-lisp-mode-map "\e\C-q" 'indent-sexp)
    (define-key inferior-lisp-mode-map "\177" 'backward-delete-char-untabify)
       (define-key inferior-lisp-mode-map "\t" 'lisp-indent-line)
