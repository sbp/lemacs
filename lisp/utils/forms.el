...
(defvar forms-use-extents (fboundp 'set-extent-property) ; lemacs 19.9+
...
(defvar forms-ro-face (if (string-match "Lucid" emacs-version)
			  'forms-label-face
			'default)
  "The face (a symbol) that is used to display read-only text on the screen.")

(defvar forms-rw-face (if (string-match "Lucid" emacs-version)
			  'forms-field-face
			'region)
...
(defvar forms--lemacs-p (string-match "Lucid" emacs-version))
...
(if forms--lemacs-p
    (progn
      ...
      ;; forms-label-face defaults to no attributes (inherit from default.)
      (make-face 'forms-field-face)
      (make-face 'forms-label-face)
      (if (face-differs-from-default-p 'forms-field-face)
	  nil
	(copy-face 'bold 'forms-field-face)
	;;(set-face-underline-p 'forms-field-face t)
	)))
...
(defun forms--process-format-list ()
  ...
  (forms--debug 'forms-format-list
		'forms--elements))

;; Special treatment for read-only segments.  (FSF19 only)
;;
    ...
    (setq 
	 ...
	 (` (lambda (arg)
	   ...
	   (` (lambda (arg)
		...
		;; After creating all the extents, set their endpoint behavior.
		;; We can't do this when creating the extents, because
		;; otherwise the text we insert for the labels would be
		;; interpreted as user input, and would alter the endpoints
		;; of the previous extents we created (the text-entry fields
		;; would be extended by the following static-text areas.)
		(map-extents
		 (function
		  (lambda (extent ignore)
		    (cond
		     ((not (extent-property extent 'forms))
		      ;; it's not one of ours; leave it alone.
		      nil)
		     ((not (extent-property extent 'read-only))
		      ;; text-entry fields should be [closed,closed] so that
		      ;; characters at either boundary go into them.
		      (set-extent-property extent 'end-open nil))
		     ;; Read-only fields should be (open,open) so that a
		     ;; read-only error isn't signalled when characters are
		     ;; inserted adjascent to them.  However, the very first
		     ;; label should be [closed,open) so that one can't
		     ;; insert text at point-min before the first label,
		     ;; and the very last should be (open,closed] for the
		     ;; same reason.
		     ((= (point-min) (extent-start-position extent))
		     ...
		     (t
		      (set-extent-property extent 'start-open t)
		      (set-extent-property extent 'end-open t)))
		    ;; return nil to continue mapping.
		    nil))
...
(defun forms--make-format-elt-using-extents (el)
  ...
  ;;                   (point)
  ;;                   (progn (insert "text: ") (point)))))
  ;;      (set-extent-face extent forms--ro-face)
  ;;      (set-extent-property extent 'read-only t)
  ;;      (set-extent-property extent 'forms t)
  ;;      )
  ;;
  ;;    ;; A field, e.g. 6.
  ;;    (let ((here (point)))
  ...
  ;;      (if (= (point) here)
  ;;          nil
  ;;        (let ((extent (make-extent here (point))))
  ;;          (set-extent-face extent forms--rw-face)
  ;;          (set-extent-property extent 'forms t)
  ;;          )))
  ;;  
  ;;    ;; A function, e.g. (tocol 40).
