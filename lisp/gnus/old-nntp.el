...
;; jwz: I've found that 400 is too big for this, but 200 works.
(defvar nntp-maximum-request 200
...
If optional argument SERVICE is non-nil, open by the service name."
  (let ((host (or host (getenv "NNTPSERVER")))
    ...
    (cond ((and host (nntp-open-server-internal host service))
	   ...
	   ;; With some INN servers, we need to do this to convince it to
	   ;; behave in the conventional way and not assume we want to do
	   ;; a batch transfer of news.  With older NNTP servers, this will
	   ;; provoke a "500 unknown command", which we can simply ignore.
	   ;; -jwz, from felix@laas.fr.
	...
	(cond ((looking-at "[23]")
	  ...
	  (progn
	    ...
	    (while wait
		  ...
		  (setq wait nil)
		(let ((k (/ (buffer-size) 1024)))
		  (gnus-lazy-message (if (> k 0)
					 "NNTP: Reading (%sk)..."
				       "NNTP: Reading...")
				     k))
		(nntp-accept-response)
		))
	    ;; Remove "reading" message if any.
	    (message nil)
