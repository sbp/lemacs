...
;;; Copyright (C) 1991, 1993, 1994 Kyle E. Jones 
;;;
...
(defun tapestry-apply-window-map (map map-width map-height current-window
				      &optional window-width window-height)
  (let ((window-min-height 1)
	(window-min-width 1)
	horizontal)
    (if (null window-width)
	(setq window-height (window-height current-window)
	...
	(if (cdr map)
	    (split-window
	     current-window
	     (if horizontal
		 (1- (/ (* (tapestry-compute-map-width (car map))
			   window-width)
			map-width))
