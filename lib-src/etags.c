...
static void *xrealloc (char *, int);
int L_isdef (char *);
int PF_funcs (FILE *);
int total_size_of_entries (NODE *);
logical consider_token (char, char *, TOKEN *, int, int, logical *);
logical tail (char *);
...
void pfnote (char *, logical, logical, char *, int, int, long);
...
/* definition of LINENO and TOKEN moved above prototypes */
...
/* definition of struct linebuffer moved to top of file,
   above prototypes */
     ...
     long cno;
{
  ...
  if (np == NULL)
      ...
      if (!emacs_tags_format)
	{
	   ...
	   * by tags.el anyway.
	   */
	  error ("too many entries to sort", (char *) 0);
	}
      put_entries (head);
      free_tree (head);
      head = NULL;
  ...
  if (emacs_tags_format)
    {
      /* Etags Mode */
      if (!last_node)
	fatal ("internal error in add_node", (char *) 0);
  ...
  if (emacs_tags_format)
    {
      if (node->rewritten)
	{
	  fprintf (outf, "%s\177%s\001%ld,%ld\n",
		   node->pat, node->name,
                   node->lno, node->cno);
	}
      else
	{
	  fprintf (outf, "%s\177%ld,%ld\n",
...
#if __STDC__ || defined(STDC_HEADERS)
logical
consider_token (register char c, register char *lp,
		register TOKEN *tokp, int c_ext, int cblev,
		logical *is_func)
#else
logical
  ...
  *is_func = FALSE;		/* not a function */
  ...
  switch (definedef)
    ...
    case dignorerest:
      return (FALSE);
    default:
      error ("internal error: definedef value", (char *) 0);
    }
     ...
     long linecharno;
{
  ...
  *s = tmpc;
  /* #### FALSE was missing; not sure if it's correct */
