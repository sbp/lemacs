...
static int MetaMask, HyperMask, SuperMask, SymbolMask;
...
#define check_modifier(name,mask) \
  if ((1<<modifier_index) != mask) \
     fprintf (stderr, "emacs: %s generates %s, which is nonsensical.\n", \
	      name, index_to_name (modifier_index))
     ...
     int simple_p;	/* means don't try too hard (ASCII only) */
{
  ...
  BLOCK_INPUT;
  XLookupString (&event->xkey, 0, 0, &keysym, 0);
  UNBLOCK_INPUT;
  ...
  default:
    ...
    UNBLOCK_INPUT;
    if (!name || !name[0]) return QKnosymbol;
  ...
  case ButtonRelease:
      ...
      case KeyPress:
	{
	  ...
	  /* More caps-lock garbage: caps-lock should *only* add the shift
	     ...
	     isn't A-Z, and if the caps lock key was down but the shift key
	     wasn't, then turn off the shift modifier.  Gag barf retch.
	   */
	  ...
	  /* If the keysym is an integer, meaning it is a graphic character,
	     then the shift bit isn't set, because it is implicit in the
	     keysym itself.  Non-graphics, like Backspace and F1 get the
	     shift bit in the modifiers slot.  Neither the characters "a",
	     "A", "2", nor "@" have the shift bit set.
	   */
	  if (XTYPE (keysym) == Lisp_Int) modifiers &= ~MOD_SHIFT;
