     ...
     Lisp_Object bytestr, vector, maxdepth;
{
  ...
  struct Lisp_String *detagged_string;
  ...
  while (1)
	...
	error ("Byte code stack underflow (byte compiler bug), pc %d",
      ...
      if (strbeg != detagged_string->data)
	{
	  pc = pc - strbeg + detagged_string->data;
          strbeg = detagged_string->data;
	}
